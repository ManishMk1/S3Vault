...
import { fetchObjects, uploadFileToS3,deleteMultipleFilesFromS3,downloadS3File } from "../Service/S3Service";
...
const Lists = () => {
    ...
    uploadFileToS3(JSON.parse(localStorage.getItem('s3Credentials')), acceptedFiles[0]);
    setObjects(prevObjects => [...prevObjects, { Key: acceptedFiles[0].name }]);
...
>
  ⬇️ Download
      ...
      <div className="mt-4">
        <button className="cursor-pointer bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg transition-transform duration-300 hover:scale-105" onClick={() => {
          const credentials = JSON.parse(localStorage.getItem('s3Credentials'));
